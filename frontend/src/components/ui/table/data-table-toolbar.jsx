...
}) {
  {
    ...
    const onFilterRender = React.useCallback(() => {
      ...
      switch (columnMeta.variant) {
        case 'text':
          return <Input placeholder={columnMeta.placeholder ?? columnMeta.label} value={column.getFilterValue() ?? ''} onChange={event => column.setFilterValue(event.target.value)} className='h-8 w-40 lg:w-56' />;
        case 'number':
          return <div className='relative'>
              <Input type='number' inputMode='numeric' placeholder={columnMeta.placeholder ?? columnMeta.label} value={column.getFilterValue() ?? ''} onChange={event => column.setFilterValue(event.target.value)} className={cn('h-8 w-[120px]', columnMeta.unit && 'pr-8')} />
